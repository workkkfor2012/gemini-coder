...
export class WebSocketManager {
  ...
  private security_token: string = 'gemini-coder';
  private client: WebSocket.WebSocket | null = null;
  private server_process: child_process.ChildProcess | null = null;
  ...
  private async initialize_server() {
    try {
      ...
      vscode.window.showErrorMessage(`Failed to initialize WebSocket server: ${error}`);
    }
  }
  ...
  private start_server_process() {
    ...
    try {
      ...
      if (this.server_process.pid) {
        this.server_process.unref();
      }

      // Log process start
      console.log(`Started WebSocket server process with PID: ${this.server_process.pid}`);
