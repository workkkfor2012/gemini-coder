...
export function completion_request_command(command: string, providerType: 'primary' | 'secondary', file_tree_provider: any, status_bar_item: vscode.StatusBarItem) {
        ...
        if (editor) {
            ...
            vscode.window.withProgress(
                ...
                async (progress) => {
                    ...
                    const text_before_cursor = document.getText(new vscode.Range(new vscode.Position(0, 0), position));
                    const text_after_cursor = document.getText(new vscode.Range(position, document.positionAt(document.getText().length)));
                    ...
                    const payload = {
                        before: `<instruction>${instruction}</instruction>\n<files>${context_text}\n<file path="${vscode.workspace.asRelativePath(document.uri)}">\n${text_before_cursor}`,
                    ...
                    async function insert_completion(completion: string) {
                        await editor!.edit((edit_builder) => {
                            ...
                            setTimeout(() => {
                                ...
                                const new_char = lines.length === 1 ? position.character + lines[0].length : lines[lines.length - 1].length;
                    ...
                    vscode.window.withProgress(
                        ...
                        async (progress) => {
                            try {
                                let completion = await make_api_request(provider, body, cancel_token_source.token);
                                ...
                                if (completion == 'rate_limit') {
                                    ...
                                    completion = await make_api_request(fallback_provider, fallback_body, cancel_token_source.token);
