...
export async function select_saved_context(
  ...
  const config_path = path.join(workspace_root, '.vscode', 'gemini-coder.json')

  // Check if the config file exists
  if (!fs.existsSync(config_path)) {
    // Create and open the example config file automatically
    const configFilePath = await create_example_config(workspace_root)
    if (configFilePath) {
...
async function apply_saved_context(
  ...
  await workspace_provider.setCheckedFiles(existing_paths)
