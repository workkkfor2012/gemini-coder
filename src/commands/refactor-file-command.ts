...
export function refactor_file_command(context: vscode.ExtensionContext, file_tree_provider: any, status_bar_item: vscode.StatusBarItem) {
    return vscode.commands.registerCommand('geminiCoder.refactorFile', async () => {
        ...
        let last_refactor_instruction = context.globalState.get<string>('lastRefactorInstruction') || '';
        ...
        const provider_type = await vscode.window.showQuickPick(['Primary', 'Secondary'], { placeHolder: 'Select type for refactoring' });
        if (!provider_type) {
            return;
        }

        const user_providers = config.get<Provider[]>('geminiCoder.providers') || [];
        const provider_name = config.get<string>(`geminiCoder.${provider_type.toLowerCase()}Model`);
        ...
        if (!provider_name || !all_providers.some((p) => p.name === provider_name)) {
            vscode.window.showErrorMessage(`${provider_type} provider is not set or invalid. Please set it in the settings.`);
            return;
        }
        ...
        provider.bearerToken = tokens_array[Math.floor(Math.random() * tokens_array.length)];
        ...
        for (const path_to_be_attached of file_paths_to_be_attached) {
            ...
            const relative_path = path.relative(vscode.workspace.workspaceFolders![0].uri.fsPath, path_to_be_attached);
        ...
        const messages = [
            ...(system_instructions ? [{ role: 'system', content: system_instructions }] : []),
            {
                role: 'user',
                content
            }
        ...
        vscode.window.withProgress(
            ...
            async (progress, token) => {
                ...
                try {
                    let refactored_content = await make_api_request(provider, body, cancel_token_source.token);

                    if (refactored_content === 'rate_limit') {
                        vscode.window.showWarningMessage('Gemini Pro has hit the rate limit. Retrying with Gemini Flash...');
                        const fallback_provider = built_in_providers.find((p) => p.name == 'Gemini Flash')!;
                        const fallback_body = {
                            ...body,
                            model: fallback_provider.model,
                            temperature: fallback_provider.temperature
                        };
                        refactored_content = await make_api_request(fallback_provider, fallback_body, cancel_token_source.token);

                        if (refactored_content === null) return;

                        if (!refactored_content) {
                            vscode.window.showErrorMessage('Fallback with Gemini Flash also failed. Please try again later.');
                            return;
                        }
                    ...
                    const fullRange = new vscode.Range(document.positionAt(0), document.positionAt(document_text.length));
                    ...
                    vscode.window.showErrorMessage('An error occurred during refactoring. See console for details.');
                }
            }
        );
    });
